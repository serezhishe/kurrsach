...
const storage = multer.diskStorage({
    ...
    filename(req, file, cb): void {
        const fileName =
            req.query.internet_tariff_name ||
            req.query.tv_tariff_name ||
            req.query.smartphone_tariff_name ||
...
app.post('/api/refreshToken', async (req, res) => {
    try {
        ...
        const user = await dbService.getUserByLogin(
            (tokenData as any).data.login
        );
        res.status(201).send({
            error: null,
...
app.post('/api/reverify', async (req, res) => {
    ...
    const response = await dbService.updateUser(
        { verify_code: randomKey },
        req.body.login
    );
...
app.patch('/api/update_user', async (req, res) => {
    ...
    const password = req.body.new_password
        ? await hash(req.body.new_password, 13)
        : undefined;
...
app.post('/api/add_abonent', async (req, res) => {
    ...
    if (req.body.package_id) {
        ...
        smartphoneTariffData = await dbService.getSmartphoneTariffById(
            packageData.smartphone_tariff_id
        );
        internetTariffData = await dbService.getInternetTariffById(
            packageData.internet_tariff_id
        );
        tvTariffData = await dbService.getTvTariffById(
            packageData.tv_tariff_id
        );
        abonentToCreate.balance -=
            Math.floor(packageData.month_pay * multiplier * 100) / 100;
        // tslint:disable: max-line-length
        const leftTariffMinuteCount =
            smartphoneTariffData.included_minute_count === -1
                ? -1
                : Math.floor(
                      smartphoneTariffData.included_minute_count * multiplier
                  );
        const leftTariffGBCount =
            smartphoneTariffData.included_gb_count === -1
                ? -1
                : Math.floor(
                      smartphoneTariffData.included_gb_count * multiplier
                  );
        const leftTariffSMSCount =
            smartphoneTariffData.included_sms_count === -1
                ? -1
                : Math.floor(
                      smartphoneTariffData.included_sms_count * multiplier
                  );
        const leftTariffHomeGBCount =
            internetTariffData.included_home_gb_count === -1
                ? -1
                : Math.floor(
                      internetTariffData.included_home_gb_count * multiplier
                  );
        abonentToCreate.left_minute_count =
            leftTariffMinuteCount === -1
                ? -1
                : abonentToCreate.left_minute_count + leftTariffMinuteCount;
        abonentToCreate.left_gb_count =
            leftTariffGBCount === -1
                ? -1
                : abonentToCreate.left_gb_count + leftTariffGBCount;
        abonentToCreate.left_sms_count =
            leftTariffSMSCount === -1
                ? -1
                : abonentToCreate.left_sms_count + leftTariffSMSCount;
        abonentToCreate.left_home_gb_count =
            leftTariffHomeGBCount === -1
                ? -1
                : abonentToCreate.left_home_gb_count + leftTariffHomeGBCount;
    } else {
        if (req.body.smartphone_tariff_id) {
            smartphoneTariffData = await dbService.getSmartphoneTariffById(
                req.body.smartphone_tariff_id
            );
            abonentToCreate.balance -=
                Math.floor(smartphoneTariffData.month_pay * multiplier * 100) /
                100;
            // tslint:disable: max-line-length
            const leftTariffMinuteCount =
                smartphoneTariffData.included_minute_count === -1
                    ? -1
                    : Math.floor(
                          smartphoneTariffData.included_minute_count *
                              multiplier
                      );
            const leftTariffGBCount =
                smartphoneTariffData.included_gb_count === -1
        ...
        if (req.body.internet_tariff_id) {
            internetTariffData = await dbService.getInternetTariffById(
                req.body.internet_tariff_id
            );
            abonentToCreate.balance -=
                Math.floor(internetTariffData.month_pay * multiplier * 100) /
                100;
            // tslint:disable: max-line-length
            const leftTariffHomeGBCount =
                internetTariffData.included_home_gb_count === -1
            ...
            abonentToCreate.left_home_gb_count =
                leftTariffHomeGBCount === -1
                    ? -1
                    : abonentToCreate.left_home_gb_count +
                      leftTariffHomeGBCount;
        }
        if (req.body.tv_tariff_id) {
            tvTariffData = await dbService.getTvTariffById(
                req.body.tv_tariff_id
            );
            abonentToCreate.balance -=
                Math.floor(tvTariffData.month_pay * multiplier * 100) / 100;
        }
    }
...
app.patch('/api/edit_abonent', async (req, res) => {
    ...
    if (req.body.package_id) {
        ...
        abonentToUpdate.left_minute_count =
            leftTariffMinuteCount === -1
                ? -1
                : abonentToUpdate.left_minute_count + leftTariffMinuteCount;
        abonentToUpdate.left_gb_count =
            leftTariffGBCount === -1
                ? -1
                : abonentToUpdate.left_gb_count + leftTariffGBCount;
        abonentToUpdate.left_sms_count =
            leftTariffSMSCount === -1
                ? -1
                : abonentToUpdate.left_sms_count + leftTariffSMSCount;
        abonentToUpdate.left_home_gb_count =
            leftTariffHomeGBCount === -1
                ? -1
                : abonentToUpdate.left_home_gb_count + leftTariffHomeGBCount;
    } else {
        if (req.body.smartphone_tariff_id) {
            smartphoneTariffData = await dbService.getSmartphoneTariffById(
                req.body.smartphone_tariff_id
